...
var STTModule = (function() {
  ...
  function micON() { // When the microphone button is clicked
    if (recording === false) {
      Conversation.sendMessage('enable voice');             // Send the message to Watson Conversation
      // setTimeout(function(){Conversation.sendMessage('voice')}, 1000);             // Send the message to Watson Conversation
      if (records === 0) { // The first time the mic is clicked - inform user
        Api.setWatsonPayload({output: {ref: 'STT'}}); // Dialog box output to let the user know we're recording
        records++;
      } else {
      ...
      recording = false;
      Conversation.sendMessage('disable voice');             // Send the message to Watson Conversation
      try {
        stream.stop();
      } catch(e){
        console.log(e);
  ...
  function pauseMic() { // When the microphone button is clicked
    if (recording === true) {
      stream.stop();
        recording = false;
    }
    ...
    fetch('/api/speech-to-text/token')        // Fetch authorization token for Watson Speech-To-Text
      ...
      .then(function(token) {                 // Pass token to Watson Speech-To-Text service
        stream = WatsonSpeech.SpeechToText.recognizeMicrophone({
          ...
          keywords: ['watson',  'home', 'home machine', 'warm', 'warm up', 'start position', 'tool align', 'tool dock', 'disable voice' ],
          ...
          inactivity_timeout: 120,              // Number of seconds to wait before closing input stream
          ...
          .then(function(data) {                        // ...put all of it into a single array
            ...
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              ...
              if ((dialogue.alternatives[0].transcript !== '') && (dialogue.final === true)) { // Another check to verify that the transcript is not empty and that this is the final dialog
                  ...
                  let keywords = ['watson',  'home', 'home machine', 'warm', 'warm up', 'start position', 'tool align', 'tool dock', 'disable voice' ];
                  let cnt = 0;
                  keywords.forEach(function(keyword){
                    if(dialogue.keywords_result.hasOwnProperty(keyword)){
                      console.log(dialogue.keywords_result);
                      cnt++;
                    }
                  });
                  console.log(cnt);
                  if(cnt > 0){
                    Conversation.sendMessage();             // Send the message to Watson Conversation using text populated in DOM element
                    return;
                  }else{
                    speechToText();
                    return;
              ...
              // Api.setWatsonPayload({output: {text: ['Microphone input Stopped']}});
