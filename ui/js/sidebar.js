...
var Sidebar = (function() {
    ...
    for (var i = 0; i < commands.length; i++) {
      var listItemJson = {
        ...
        'children': [{
          ...
          'attributes': [{
            'name': 'onclick',
            'value': 'Sidebar.toggle(); Conversation.sendMessage("' + commands[i] + '")'
          }]
        }]
      };
  ...
  function populateRoutines() {
    var routineList = document.getElementById(ids.routineList);
    for (var i = 0; i < routines.length; i++) {
      var listItemJson = {
        ...
        'children': [{
          ...
          'attributes': [{
            'name': 'onclick',
            'value': 'Sidebar.toggle(); Conversation.sendMessage("' + routines[i] + '")'
          }]
        }]
      };
      routineList.appendChild(Common.buildDomElement(listItemJson));
    }
  }
